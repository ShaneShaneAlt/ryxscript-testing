// RyxScript.js - The All-in-One Runtime & Transpiler v0.1.4
// (c) 2023 [Your Name] - For demonstration purposes
// CUMULATIVE FIX: Contains all previous fixes and adds one for proper type instantiation (e.g., Color(...)).
...
(function() {
    ...
    const Ryx = {
        ...
        Entity: class Entity {
            ...
            on(eventName, data) {
                ...
                if (typeof this[handlerName] === 'function') { this[handlerName](data); }
            }
        ...
        emit: function(eventName, data) {
            for (const entity of this.entities) { entity.on(eventName, data); }
        },

        Gfx: {
            setResolution(w, h) { Ryx.canvas.width = w; Ryx.canvas.height = h; },
            get width() { return Ryx.canvas.width; },
            get height() { return Ryx.canvas.height; },
            setBackground(color) { Ryx.ctx.fillStyle = color.toString(); Ryx.ctx.fillRect(0, 0, this.width, this.height); },
            ...
            drawImage(image, pos) { Ryx.ctx.drawImage(image, pos.x - image.width / 2, pos.y - image.height / 2); }
        },
        ...
        Input: {
            ...
            getVector(left, right, up, down) {
                let x = 0, y = 0;
                if (this._keys.has(left)) x -= 1; if (this._keys.has(right)) x += 1;
                if (this._keys.has(up)) y -= 1; if (this._keys.has(down)) y += 1;
                const len = Math.sqrt(x*x + y*y);
                if (len > 0) { return new Ryx.Vector2(x / len, y / len); }
        ...
        _gameLoop: function(timestamp) {
            ...
            for (let i = this.entities.length - 1; i >= 0; i--) {
                ...
                if (entity._isDestroyed) { this.entities.splice(i, 1); } else { entity.tick(this.deltaTime); }
            }
            for (const entity of this.entities) { entity.render(); }
            requestAnimationFrame(this._gameLoop.bind(this));
        },

        _init: function() {
            this.canvas = document.getElementById('ryx-canvas');
            if (!this.canvas) { console.error("RyxScript Error: Canvas with id 'ryx-canvas' not found!"); return; }
    ...
    function transpile(ryxCode) {
        ...
        jsCode = jsCode.replace(/\bentity\s+(\w+)([\s\S]*?)end/g, (match, name, content) => {
            ...
            // Handles `init` blocks, correctly stripping parameter types.
            ...
            // Handles `on` event blocks, correctly stripping parameter types.
        ...
        // NEW FIX: Handle built-in type instantiation like `Color(...)` and `Vector2(...)`.
        jsCode = jsCode.replace(/\b(Color|Vector2)\((.*?)\)/g, 'new Ryx.$1($2)');
        
        jsCode = jsCode.replace(/\bspawn\s*\((.*?)\)/g, 'Ryx.spawn($1)');
        
        // Final cleanup
        jsCode = jsCode.replace(/<[\w\d\/]+>/g, ''); // Strip unit types
        jsCode = jsCode.replace(/(\b[A-Z]\w+)::/g, 'Ryx.$1.'); // Scope static calls
    ...
    Ryx._loadAndTranspileScripts = async function() {
        ...
        for (const script of scripts) {
            try {
                ...
                if (!response.ok) { throw new Error(`Failed to load script: ${script.src}`); }
    ...
    // 4. AUTO-INITIALIZE
    // ========================================================================
    document.addEventListener('DOMContentLoaded', () => { Ryx._init(); });
